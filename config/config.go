...
func (conf *Config) SaveConfig() (err error) {
	...
	logs.Info("配置文件已保存在: %s", configFilePath)
...
func (conf *Config) ResetPassword(newPassword string) {
	...
	logs.Info("用户名 %s 的密码已重置成功! 请重启ddns-go", conf.Username)
}
...
func (conf *Config) CheckPassword(newPassword string) (hashedPwd string, err error) {
	...
	if err != nil {
		return "", errors.New("密码不安全！尝试使用更复杂的密码")
	}

	// 加密密码
	hashedPwd, err = util.HashPassword(newPassword)
	if err != nil {
		return "", fmt.Errorf("异常信息: %s", err.Error())
	}
	return
}
...
func (conf *DnsConfig) getIpv4AddrFromInterface() string {
	...
	logs.Info("从网卡中获得IPv4失败! 网卡名: %s", conf.Ipv4.NetInterface)
	return ""
}
...
func (conf *DnsConfig) getIpv4AddrFromUrl() string {
	...
	for _, url := range urls {
		...
		if err != nil {
			logs.Info("通过接口获取IPv4失败! 接口地址: %s", url)
			logs.Info("异常信息: %s", err)
			continue
		}
		...
		if result == "" {
			logs.Info("获取IPv4结果失败! 接口: %s ,返回值: %s", url, string(body))
		}
		return result
	}
	return ""
...
func (conf *DnsConfig) getAddrFromCmd(addrType string) string {
	...
	if err != nil {
		logs.Info("获取%s结果失败! 未能成功执行命令：%s, 错误：%q, 退出状态码：%s", addrType, execCmd.String(), out, err)
		return ""
	}
	str := string(out)
	// get result
	...
	if result == "" {
		logs.Info("获取%s结果失败! 命令: %s, 标准输出: %q", addrType, execCmd.String(), str)
	}
	return result
}
				...
				if match, err := regexp.MatchString("@\\d", conf.Ipv6.Ipv6Reg); err == nil && match {
					...
					if err == nil {
						if num > 0 {
							...
							logs.Info("未找到第 %d 个IPv6地址! 将使用第一个IPv6地址", num)
							return netInterface.Address[0]
						}
						logs.Info("IPv6匹配表达式 %s 不正确! 最小从1开始", conf.Ipv6.Ipv6Reg)
						return ""
					}
				}
				// 正则表达式匹配
				logs.Info("IPv6将使用正则表达式 %s 进行匹配", conf.Ipv6.Ipv6Reg)
				for i := 0; i < len(netInterface.Address); i++ {
					matched, err := regexp.MatchString(conf.Ipv6.Ipv6Reg, netInterface.Address[i])
					if matched && err == nil {
						logs.Info("匹配成功! 匹配到地址: ", netInterface.Address[i])
						return netInterface.Address[i]
					}
				}
				logs.Info("没有匹配到任何一个IPv6地址, 将使用第一个地址")
			}
			return netInterface.Address[0]
		}
	}

	logs.Info("从网卡中获得IPv6失败! 网卡名: %s", conf.Ipv6.NetInterface)
	return ""
}
...
func (conf *DnsConfig) getIpv6AddrFromUrl() string {
	...
	for _, url := range urls {
		...
		resp, err := client.Get(url)
		if err != nil {
			logs.Info("通过接口获取IPv6失败! 接口地址: %s", url)
			logs.Info("异常信息: %s", err)
			continue
		}
		...
		if result == "" {
			logs.Info("获取IPv6结果失败! 接口: %s ,返回值: %s", url, result)
		}
		return result
	}
	return ""
