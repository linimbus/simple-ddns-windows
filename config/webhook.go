	...
	if conf.WebhookURL != "" && (v4Status != UpdatedNothing || v6Status != UpdatedNothing) {
		...
		if v4Status == UpdatedFailed || v6Status == UpdatedFailed {
			...
			if updatedFailedTimes != 3 {
				logs.Info("将不会触发Webhook, 仅在第 3 次失败时触发一次Webhook, 当前失败次数：%d", updatedFailedTimes)
				return
			}
		} else {
		...
		if err != nil {
			logs.Info("Webhook调用失败! 异常信息：%s", err)
			return
		}
		...
		if err == nil {
			logs.Info("Webhook调用成功! 返回数据：%s", string(body))
		} else {
			logs.Info("Webhook调用失败! 异常信息：%s", err)
		}
	}
...
func extractHeaders(s string) map[string]string {
	...
	for _, line := range lines {
		...
		if len(parts) != 2 {
			logs.Info("Webhook Header不正确: %s", line)
			continue
		}
