...
func (cb *Callback) addUpdateDomainRecords(recordType string) {
	...
	if recordType == "A" {
		if cb.lastIpv4 == ipAddr {
			logs.Info("你的IPv4未变化, 未触发 %s 请求", "Callback")
			return
		}
	} else {
		if cb.lastIpv6 == ipAddr {
			logs.Info("你的IPv6未变化, 未触发 %s 请求", "Callback")
			return
		}
	}
	...
	for _, domain := range domains {
		...
		if err == nil {
			logs.Info("Callback调用成功, 域名: %s, IP: %s, 返回数据: %s", domain, ipAddr, string(body))
			domain.UpdateStatus = config.UpdatedSuccess
		} else {
			logs.Info("Callback调用失败, 异常信息: %s", err)
			domain.UpdateStatus = config.UpdatedFailed
		}
	}
}
