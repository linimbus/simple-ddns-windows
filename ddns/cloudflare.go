...
func (cf *Cloudflare) addUpdateDomainRecords(recordType string) {
	...
	for _, domain := range domains {
		...
		if len(result.Result) == 0 {
			logs.Info("在DNS服务商中未找到根域名: %s", domain.DomainName)
			domain.UpdateStatus = config.UpdatedFailed
			return
		}
		...
		if !records.Success {
			logs.Info("查询域名信息发生异常! %s", strings.Join(records.Messages, ", "))
			domain.UpdateStatus = config.UpdatedFailed
			return
		}
...
func (cf *Cloudflare) create(zoneID string, domain *config.Domain, recordType string, ipAddr string) {
	...
	if status.Success {
		...
		logs.Info("新增域名解析 %s 失败! 异常信息: %s", domain, strings.Join(status.Messages, ", "))
		domain.UpdateStatus = config.UpdatedFailed
	}
}
...
func (cf *Cloudflare) modify(result CloudflareRecordsResp, zoneID string, domain *config.Domain, ipAddr string) {
	for _, record := range result.Result {
		...
		if status.Success {
			...
			logs.Info("更新域名解析 %s 失败! 异常信息: %s", domain, strings.Join(status.Messages, ", "))
			domain.UpdateStatus = config.UpdatedFailed
		}
	}
}
