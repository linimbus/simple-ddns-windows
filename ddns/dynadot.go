...
func (dynadot *Dynadot) addOrUpdateDomainRecords(recordType string) {
	...
	if recordType == "A" {
		if dynadot.LastIpv4 == ipAddr {
			logs.Info("你的IPv4未变化, 未触发 %s 请求", "dynadot")
			return
		}
	} else {
		if dynadot.LastIpv6 == ipAddr {
			logs.Info("你的IPv6未变化, 未触发 %s 请求", "dynadot")
			return
		}
	}
	...
	if len(records) != 1 {
		logs.Info("dynadot仅支持单域名配置，多个域名请添加更多配置")
		return
	}
...
func (dynadot *Dynadot) createOrModify(record *DynadotRecord, recordType string, ipAddr string) {
	...
	for _, domain := range domains {
		...
		if result.ErrorCode != -1 {
			...
			logs.Info("更新域名解析 %s 失败! 异常信息: %s", domain, strings.Join(result.Content, ","))
			domain.UpdateStatus = config.UpdatedFailed
		}
	}
